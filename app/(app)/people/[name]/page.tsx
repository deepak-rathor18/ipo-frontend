...
const STATUS_LABELS: Record<PersonSummary["status"], string> = {
  RECEIVABLE: "You'll receive",
  PAYABLE: "You owe",
  SETTLED: "Settled",
};

const STATUS_VARIANT: Record<PersonSummary["status"], "profit" | "loss" | "secondary"> = {
  RECEIVABLE: "profit",
  PAYABLE: "loss",
  SETTLED: "secondary",
};
...
export default function PersonDetailPage() {
  ...
  if (error || !summary) {
    return <ErrorState message={error ?? "Person not found."} onRetry={load} />;
  }
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <Badge variant={STATUS_VARIANT[summary.status]}>{STATUS_LABELS[summary.status]}</Badge>
          </div>
          {summary.phone && <p className="mt-1 text-sm text-muted-foreground">{summary.phone}</p>}
        </div>
      </div>

      <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
        <Card className="p-4">
          <p className="text-xs font-medium text-muted-foreground">To Receive</p>
          <p className="mt-1.5 text-xl font-semibold text-profit">
            {formatCurrency(summary.toReceive)}
          </p>
        </Card>
        <Card className="p-4">
          <p className="text-xs font-medium text-muted-foreground">To Pay</p>
          <p className="mt-1.5 text-xl font-semibold text-loss">{formatCurrency(summary.toPay)}</p>
        </Card>
        <Card className="p-4">
        ...
        {transactions.length === 0 ? (
          <div className="p-4">
            <EmptyState
              ...
              description={`No money transactions recorded for ${summary.name} yet.`}
